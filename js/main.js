...
//    this.x = rectRange.width * i + rectRange.leftSpace;
  ...
  draw(y,janken) {
    ...
    img.onload = function(e){
      ctx.layer.drawImage(img, this.x + rectRange.width, y ,rectRange.height*1.5,rectRange.height*1.5);
      ctx.layer.drawImage(img, this.x + rectRange.width*2, y ,rectRange.height*1.5,rectRange.height*1.5);
    }
    img.src = IMG[janken];
  }
}
...
const setInput = (line) => {
  ...
  if((line === 0) && ((max_class === 0) && (notes.line[notes.index] === 1)) || ((max_class === 1) && (notes.line[notes.index] === 2)) || ((max_class === 2) && (notes.line[notes.index] === 0))){
    for (let i = 0; i < JUDGE.size; ++i) {
      if (inputRange.top[i] < y && y < inputRange.bottom[i]) {
        playData.judge = i;
        break;
